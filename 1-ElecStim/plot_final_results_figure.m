...
%   - 使用彩色曲线以便于区分
    ...
    plot_subplot(7, cellfun(@(x) x.IKCa, results, 'UniformOutput', false), '(pA/pF)', 'I_{KCa}', [-0.02, 0.10], 1/Cm);
    ...
    plot_subplot(12, cellfun(@(x) x.JNKCC1 * -1000, results, 'UniformOutput', false), '(\muM)', 'J_{NKCC1}', [23, 25], 1);
%% 第4行
% 绘图 J_KCC2，单位从 mM 转换为 µM
    plot_subplot(13, cellfun(@(x) x.JKCC2 * 1000, results, 'UniformOutput', false), '(\muM)', 'J_{KCC2}', [-28.14, -28.015], 1);
    ...
    plot_subplot(19, cellfun(@(x) x.Caos, results, 'UniformOutput', false), '(\muM)', '[Ca^{2+}]_{os}', [0.000, 0.120], 1);
    plot_subplot(20, cellfun(@(x) x.Cais, results, 'UniformOutput', false), '(\muM)', '[Ca^{2+}]_{is}', [0.024, 0.072], 1);
    
    %% 第6行
    plot_subplot(21, cellfun(@(x) x.cGMP, results, 'UniformOutput', false), '(\muM)', '[cGMP]', [0.0, 8.0], 1);
