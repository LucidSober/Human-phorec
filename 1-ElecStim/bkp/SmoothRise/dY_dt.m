...
% dt=1e6; % 步长不匹配，要调节
% stim
if( (params.Stim ==1) && (t>=(params.stim_start-params.pre_stim_start) ))
    te=t-(params.stim_start-params.pre_stim_start);
    Rs=params.Rs;
    Rf=params.Rf;
    Cdl=params.Cdl;
    Ve_mV =states_r.Ve;
    ...
    dVe_dt_V =  Rs * (I_func(te+dt) - I_func(te)) + (I_func(te) - (Ve_V - I_func(te)*Rs)/Rf)/Cdl;
    dVe_dt_mV=dVe_dt_V*1000;
else
    dVe_dt_mV=0;
end

I_total_A = ion_currents.I_total * 1e-12; % pA -> A
dVi_dt_V = -I_total_A / (params.Cm * 1e-12); % A / F -> V/s
dVi_dt_mV = dVi_dt_V * 1000; % V/s -> mV/s

% --- 膜电位导数 (dVm/dt) ---
dVm_dt_mV =dVi_dt_mV-dVe_dt_mV; % = d(Vi(t)-Ve(t))/dt
...
Y_prime(95) = dVe_dt_mV; % Ve
Y_prime(96) = dVi_dt_mV; % Vi
